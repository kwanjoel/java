...
class Container {
	...
	/**
	 * synchronized method for setting the resource: The code in the get method
	 * loops until the Producer has produced a new value. Each time through the
	 * loop, the get method calls the wait method; The wait method relinquishes the
	 * lock held by the Consumer on the Container (thereby allowing the Producer to
	 * get the lock and update the Container) and then waits for notification from
	 * the Producer.
	 */
	public synchronized void setSharedResource(int val) {
		...
		System.err.println(Thread.currentThread().getName() + " generates Resource value number = " + val);
		sharedInt = val;
		writeable = false;
	...
	/**
	 * synchronized method for getting the resource: When the Producer puts
	 * something in the Container, it notifies the Consumer by calling notifyAll.
	 * The Consumer then comes out of the wait state, the loop exits, and the get
	 * method returns the value in the Container. The set method works in a similar
	 * fashion, waiting for the Consumer thread to consume the current value before
	...
	public synchronized int getSharedResource() {
		...
		writeable = true;
		notify(); // tell a waiting thread to become ready
		System.err.println(Thread.currentThread().getName() + " uses Resource value number = " + sharedInt + "\n");
		return sharedInt;
	}
}
